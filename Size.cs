...
namespace LottieJsonGenerator
{
    ...
    public class Size
    {
        ...
        // Static size or a list of keyframes if animated
        [JsonPropertyName("k")]
        public object K { get; set; } = new List<Keyframe>
        {
            new Keyframe
            {
                Time = 0,
        ...
        [JsonPropertyName("x")]
        public string Expression { get; set; } // Optional expression for dynamic calculations

        // Method to set animated size with keyframes
        public void SetAnimatedSize(List<Keyframe> keyframes)
        {
            Animated = 1;
            K = keyframes;
        }
